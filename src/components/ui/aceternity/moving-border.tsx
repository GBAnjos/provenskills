    ...
    >
      ...
      >
        <MovingBorder duration={duration} rx="30%" ry="30%">
          <div
            className={cn(
              "h-20 w-20 bg-[radial-gradient(var(--proven-green-500)_40%,transparent_60%)] opacity-[0.8]",
              borderClassName
            )}
          />
        </MovingBorder>
      ...
      <div
        className={cn(
          "relative flex h-full w-full items-center justify-center border border-border bg-card text-sm antialiased backdrop-blur-xl",
          className
        )}
        style={{
...
export function MovingBorder({
  ...
  React.useEffect(() => {
    if (pathRef.current) {
      ...
      if (pathLength) {
        const controls = animate(progress, pathLength, {
